...
(function ($) {
	...
	AblePlayer.prototype.getPreferencesGroups = function() {
		...
		else if (!this.hasCaptions && this.hasClosedDesc) {
			return ['keyboard', 'descriptions']; 
		}
		else if (!this.hasCaptions) {
			return ['keyboard']; 
		}
		else if (this.mediaType === 'audio'){
			return ['captions','keyboard','transcript']; 
		}
		else { 
			return ['captions','keyboard','transcript']; 
		} 
		...
		if (which === 'prefs') {
			if (this.prefCats.length > 1) {
				for (i = 0; i < this.prefCats.length; i++) {
					...
					else if (prefCat === 'descriptions' && this.mediaType === 'video') {
...
(function ($) {
	...
	AblePlayer.prototype.getYouTubeCaptionTracks = function (youTubeId) {
		...
		//ADDED: this flag for onApiChange event, changed to true inside the evt
		var apiTriggered = false;

		//ADDED: setTimeout placed below playVideo()
		...
		setTimeout(() => {
			if(promise.state() === 'pending' && !apiTriggered){
					...
					setTimeout(() => {
						...
						console.log('ableplayer 4.5: Resolving after second timeout.');
...
(function ($) {
	...
	AblePlayer.prototype.playMedia = function () {
		...
		AblePlayerInstances.forEach(function (instance) {
				var currentAbleid = instance.$media.attr('id');
				AblePlayerInstances.forEach(function (instance) {
					if (currentAbleid != instance.$media.attr('id')) {
						if (instance.player === 'html5') {
							instance.media.pause(true);
							if (instance.hasSignLanguage && this.signVideo) {
								instance.signVideo.pause(true);
							}
						}
						else if (instance.player === 'youtube') {
							instance.youTubePlayer.pauseVideo();
						}
						else if (instance.player === 'vimeo') {
							instance.vimeoPlayer.pause();
						}					}
				});
		});
	...
	AblePlayer.prototype.handleRewind = function() {
		...
		var tHours = Math.floor(targetTime / 3600);
		var tMinutes = Math.floor((targetTime % 3600) / 60);
		var tSeconds = Math.floor(targetTime % 60);

		var tHourWord = tHours === 1 ? 'hour' : 'hours';
		var tMinuteWord = tMinutes === 1 ? 'minute' : 'minutes';
		var tSecondWord = tSeconds === 1 ? 'second' : 'seconds';

		var tdescriptionText;

		if (tHours > 0) {
			tdescriptionText = tHours +
				' ' + tHourWord +
				', ' + tMinutes +
				' ' + tMinuteWord +
				', ' + tSeconds +
			...
			tdescriptionText = tMinutes +
				' ' + tMinuteWord +
				', ' + tSeconds +
				' ' + tSecondWord;
		}
		else {
			tdescriptionText = tSeconds + ' ' + tSecondWord;
		}		
		if (!this.$controllerDiv.liveAriaRegion) {
			this.$controllerDiv.liveAriaRegion = $('<span>', {
				'class': 'able-offscreen',
				'aria-live': 'assertive',
				'aria-atomic': 'true'
			});

			this.$controllerDiv.append(this.$controllerDiv.liveAriaRegion);
		}
		if (this.$controllerDiv.liveAriaRegion.text() !== tdescriptionText){
			this.$controllerDiv.liveAriaRegion.text(tdescriptionText);		
		}
